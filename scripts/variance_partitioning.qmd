...
graph_part_var <- function(levels,
    ...
    var_by_list <- lapply(by_levels, function(x) {
        ...
        model <- lmer(formula, data = dat, control = lmerControl(optimizer = "bobyqa", 
...
graph_part_var(levels = c("Site", "UniqueBird"), data = as.data.frame(tbp_calls), by = "type", response = "mds1.xc", full = FALSE)
